...
def fetch_update_max_call_value():
    ...
    if data[last_date]:
        ...
        print(f"No good options today. Nothing to report.")
        options_data = {}
    ...
    if not data[last_date]:
        ...
        telegram_text += f"No good options today. Nothing to report."
        ...
        for option_id, details in options_data.items():
            ...
            if open_price > 0:  # Avoid division by zero
                ...
                if performance < 30:
                    performance_str = f"🔴 {performance:+.2f}% (DID NOT REACH 30%. Consider -20%)"
                else:
